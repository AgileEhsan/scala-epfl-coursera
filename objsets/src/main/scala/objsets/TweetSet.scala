...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    ...
    def unionAcc(that: TweetSet, acc: TweetSet): TweetSet =
    if(that.contains(this.elem)) right.unionAcc(that, left.unionAcc(that, acc))
    else right.unionAcc(that, left.unionAcc(that, acc.incl(this.elem)))
      
  def mostRetweetedAcc(acc: Tweet): Tweet =
  	if(this.elem.retweets < acc.retweets)
  	  right.mostRetweetedAcc(left.mostRetweetedAcc(this.elem))
  ...
  override def toString: String = elem + " L:" + left.toString() + " R:" + right.toString()
